module Jellyfish.SuperCombinator.Semantics.WithoutToplevels where
...
mutual
  ...
  eval ρ (cls e x⃗)      = (_ , map (find ρ) x⃗ , e)
...
data _⊢_⇓_ {Γ} (ρ : Env Γ) : ∀ {A} → Γ ⊢ A → ⟦ A ⟧ → Set where
  ...
  cls : ∀ {A⃗ A B} {e : A⃗ ▻ A ⊢ B} {x⃗ : Γ ∋⃗ A⃗}
    ----------------------------------------
    → ρ ⊢ cls e x⃗ ⇓ (_ , map (find ρ) x⃗ , e)
